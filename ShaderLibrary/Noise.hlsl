...
// @param Tiling = true to tile, false to not tile
// @param RepeatSize = number of units before repeating
// @return either original or wrapped coord
float3 NoiseTileWrap(float3 v, bool Tiling, float RepeatSize)
{
    return Tiling ? (frac(v / RepeatSize) * RepeatSize) : v;
}
...
// @param Tiling = true to return seed values for a repeating noise pattern
...
float3 NoiseSeeds(float3 v, bool Tiling, float RepeatSize,
...
// @param v = 3D noise argument, use float3(x,y,0) for 2D or float3(x,0,0) for 1D
// @param Tiling = repeat noise pattern
// @param RepeatSize = integer units before tiling in each dimension
// @return random number in the range -1 .. 1
float GradientNoise3D(float3 v, bool Tiling, float RepeatSize)
{
    float seed000, seed001, seed010, seed011, seed100, seed101, seed110, seed111;
    float3 fv = NoiseSeeds(v, Tiling, RepeatSize, seed000, seed001, seed010, seed011, seed100, seed101, seed110, seed111);
...
float FractalNoise3D(float3 v, bool Tiling, float RepeatSize, int Octaves = 8, float Lacunarity = 2.0, float Persistence = 0.5, float BaseFrequency = 1.0, float BaseAmplitude = 1.0)
{
    float sum = 0;
    ...
    {
        sum += GradientNoise3D(v * freq, Tiling, RepeatSize) * amp;
